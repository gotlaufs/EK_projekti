...
/* pin defs */
/* REDEFINE THESE FOLLOWING THINGS */
...
void goto_x(uint8_t X);
void goto_y(uint8_t Y);
 ...
 */
inline void waitForHi(uint32_t* reg, uint32_t mask)
{
    while(!(*reg & mask))
        __asm__("nop");
}
 ...
 * @param mask Bitmask of bits to wait for
 */
inline void waitForLo(uint32_t* reg, uint32_t mask)
{
    while(*reg & mask)
        __asm__("nop");
}
...
/* --- LCD STUFF --- */
// TODO: Move CLR_RESET, SET_CS, SET_COMMAND etc. to this class
class 5110_LCD {
    uint8_t x, y;
    uint8_t ram[84][48];
public:
    void init_lcd();
    void goto_x(uint8_t x_address);
    void goto_y(uint8_t y_address);
    void set_pixel(uint8_t x, uint8_t y);
    void clr_pixel(uint8_t x, uint8_t y);
    void refresh_block(uint8_t x, uint8_t y);
} lcd;
...
void 5110_LCD::goto_x(uint8_t x_address)
{
    this->x = x_address;
    waitForLo(&SPI->SR, 0x80);  /* Wait while SPI is busy transmitting data */
    SET_COMMAND;
...
void 5110_LCD::goto_y(uint8_t y_address)
{
    this->y = y_address;
    waitForLo(&SPI->SR, 0x80);  /* Wait while SPI is busy transmitting data */
    SET_COMMAND;
...
void 5110_LCD::write_data_fast(uint8_t data)
{
    waitForLo(&SPI->SR, 0x80);  /* Wait while SPI is busy transmitting data */
    SET_DATA;
    SPI->DR = data;
...
void 5110_LCD::write_data(uint8_t data)
{
    this->ram[this->x][this->y] = data;
    this->x++;
    if( this->x > 83 ){
        this->x = 0;
        this->y++;
...
void 5110_LCD::set_Pixel(uint8_t x, uint8_t y)
{
    uint8_t row = y >> 3;
    uint8_t bit = 0x01 << (y & 0x07);
    this->goto_x(x);
    this->goto_y(row);
    writeData( this->ram[x][row] | bit );
}

void 5110_LCD::clr_pixel(uint8_t x, uint8_t y)
{
    uint8_t row = y >> 3;
    uint8_t bit = 1 << (y & 0x07);
    this->goto_x(x);
    this->goto_y(row);
    writeData( this->ram[LCD_X][LCD_Y] & ~bit );
}

void 5110_LCD::refresh_block(uint8_t x, uint8_t y)
{
    this->goto_x(x);
    this->goto_y(y);
    this->writeDataFast(this->ram[this->x][this->y]);
}
